...
\begin{exercice_cours}[{Manipulation de base}]{}
	...
	\begin{enumerate}
		...
		\item Modifier le deuxième élément de cette liste pour qu'il soit égal à 10. (\emph{affiche [4, 10, 7, 9, 4]})
		\item Modifier le dernier élément de cette liste pour qu'il soit égal à 5. (\emph{affiche [4, 10, 7, 9, 5]})
		\item Modifier le premier éléments de la liste en le remplaçant par la somme du premier et du troisième élément de cette liste. (\emph{affiche [11, 10, 7, 9, 5]})
...
\begin{easybox}{Itérer sur une liste avec une boucle FOR}{}
	...
	Comme pour les chaines de caractères, On peut itérer sur les éléments d'une liste à l'aide d'une boucle \verb|FOR|.	
...
print(ma_liste)		# Affiche: [0, 1, 2, 3, 4]
		\end{CodePythontexAlt}
		\end{easybox}
...
\item Ecrire une fonction \texttt{est\_triee(liste)} qui renvoie \texttt{True} si la liste \texttt{liste} est triée dans l'ordre croissant, et \texttt{False} sinon.
					\begin{CodePythontexAlt}[TaillePolice=\normalsize, Largeur=1\linewidth,Centre,PremLigne=1, Lignes=false]{}
def est_triee(liste):
	''' Renvoie True si liste est triée dans l'ordre croissant et False sinon'''
...
print(ma_liste)			# Affiche: [1, 2, 3, 4, 5, 6]
	 ...
	 Ecrire une fonction \texttt{decalage\_gauche(liste)} qui décale les éléments de la liste \texttt{liste} vers la gauche, en mettant le premier élément à la fin de la liste. La liste doit être modifiée en place.
			...
			\end{exercice_cours}
\begin{exercice_cours}[Les listes]{}

Ecrire une fonction qui renvoie une nouvelle liste contenant la somme des éléments de deux listes de même longueur. La fonction doit vérifier que les deux listes ont la même longueur. Si les deux listes ne sont pas de même longueur, la liste la plus courte est complétée par des zéros jusqu'à ce qu'elle ait la même longueur que l'autre liste.
\begin{CodePythontexAlt}[TaillePolice=\normalsize, Largeur=1\linewidth,Centre,PremLigne=1, Lignes=false]{}
def somme_listes(L1, L2):
	''' Renvoie une nouvelle liste contenant la somme des éléments de L1 et L2'''
...
assert somme_listes([1, 2, 3], [4, 5, 6]) == [5, 7, 9]
assert somme_listes([1, 2], [3, 4, 5]) == [4, 6, 5]
\end{CodePythontexAlt}
\end{exercice_cours}
...
\begin{exercice_cours}[Les listes]{}
	...
	Ecire une fonction qui modifie une liste en remplaçant chaque élément par la sommede tous les éléments qui sont à sa droite. Par exemple, si la liste est \verb|[1, 2, 3, 4]|, la liste modifiée doit être \verb|[10, 9, 7, 4]|. La fonction doit modifier la liste en place.

\begin{CodePythontexAlt}[TaillePolice=\normalsize, Largeur=1\linewidth,Centre,PremLigne=1, Lignes=false]{}
def somme_droite(liste):
	''' Modifie la liste en remplaçant chaque élément par la somme de tous 
	les éléments à sa droite'''
...
assert somme_droite([1, 2, 3, 4]) == [10, 9, 7, 4]
assert somme_droite([5, 4, 3, 2, 1]) == [15, 10, 6, 3, 1]
\end{CodePythontexAlt}
\end{exercice_cours}
