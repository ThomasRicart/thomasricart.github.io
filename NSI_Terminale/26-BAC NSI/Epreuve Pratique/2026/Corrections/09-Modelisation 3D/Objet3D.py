...
#############################################################################
    def transformer(self, rapport):
        ...
        nouvel_objet.sommets = sommets
        nouvel_objet.faces = self.faces
        return nouvel_objet
...
#############################################################################
    ...
    def trouver_sommets_adjacents(self):
        # comme un sommet n'est pas adjacent à lui-même
        # d'après est_adjacent, on peut parcourir deux fois
        # la liste sans précaution
        for sommet1 in self.sommets:
            for sommet2 in self.sommets:
                if sommet1.est_adjacent(sommet2):
