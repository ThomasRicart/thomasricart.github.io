...
def test_validation():
    assert validation_tel("01236514985") == False
    assert validation_tel("0123651498") == False
    assert validation_tel("3123651498") == False
    assert validation_tel("4623651498") == False
    assert validation_tel("0623651498") == True
    assert validation_tel("0723651498") == True
    print("Les test de la fonction validation_tel sont passés")
...
def consultation_vaccination_chat(date):
    ...
    FROM animal
        JOIN consultation ON consultation.id_animal = animal.id
        JOIN proprietaire ON proprietaire.id = animal.id_proprietaire 
    WHERE animal.espece = 'chat' AND consultation.motif = 'vaccination' AND consultation.date > ?
