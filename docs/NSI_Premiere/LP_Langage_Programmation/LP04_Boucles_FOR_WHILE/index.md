# 🐍 LP04 : Les Boucles FOR / WHILE

<div class="subtitle" style="font-size: 1.2em; color: #546e7a; margin-bottom: 25px; font-style: italic;">
</div>

---

## 📂 Ressources du Chapitre

Accédez ici à l'ensemble des documents de cours et aux outils de révision interactive.

| Support | Description | Corrections |
| :--- | :--- | :--- |
| [📕 **Cours**](./LP04_Boucles_FOR_WHILE.pdf) | Le support de cours complet en version PDF. | |
| [💡 **Synthèse**](./LP04_Boucles_FOR_WHILE_Synthese.html) | Fiche récapitulative des concepts clés (HTML). | |
| [🎴 **Flashcards**](./flashcards/LP04_flashcards.html) | Cartes de mémorisation pour réviser les notions. ||
| [📝 **Quiz**](./quiz/LP04_quiz.html)| Auto-évaluation rapide sur le chapitre. ||
| [✏️ **Exercices 01**](./exercices/LP04_EXO1_Sujet.pdf) | Exercices d'application boucle FOR |[Correction](./exercices/LP04_EXO1_Correction.pdf) |
| [✏️ **Exercices 02**](./exercices/LP04_EXO2_Sujet.pdf) | Exercices d'application boucle WHILE |[Correction](./exercices/LP04_EXO2_Correction.pdf) |
| [📚 **Activité pratique 1**](./LP04-ACT1-boucles_FOR.ipynb) | Activité Capytale Boucle FOR | [Correction](./)|
| [📚 **Activité pratique 2**](./LP04-ACT2-boucles_WHILE.ipynb) | Activité Capytale Boucle WHILE | [Correction](./)|

---
---

### 🧩 Concepts illustrés
```python
```