...
\begin{enumerate}[1)]
    \item 1 bit  -- 2 états
    \item 3 bits -- $2^3=8$ états 
    \item 8 bits -- $2^8=256$ états 
...
\begin{answer}{1}{2} 
    ...
    $N = 1\times2^7 + 0\times2^6 + 1\times2^5 + 0\times2^4 + 0\times2^3 + 0\times2^2 + 1\times2^1 + 1\times2^0 = 128 + 32 + 2 + 1 = 163$
\end{answer}
\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}
     \exods{\ptsexoC}
...
\end{center}   
    ...
    $159 = 128 + 16 + 8 + 4 + 2 + 1 = 2^7 + 2^4 + 2^3 + 2^2 + 2^1 + 2^0$
    
Donc $159_{10} = 10011111_2$
\end{answer}
\end{minipage}
...
\begin{answer}{1}{2} 
    ...
    $N = 10\times16^1 + 15\times16^0 = 160 + 15 = 175$
...
\begin{answer}{1}{2} 
    ...
    $1110_2 = 14_{10} = E_{16}$ et $0110_2 = 6_{10} = 6_{16}$

    Donc $11100110_2 = E6_{16}$
\end{answer}
\end{minipage}
...
Expliquer simplement comment déterminer si un nombre binaire est pair ou impair. Justifier.
    ...
    \begin{answer}{1}{3}   
    Un nombre binaire est pair si son bit de poids faible (le bit le plus à droite) est égal à 0, et impair s'il est égal à 1.

    En effet, le bit de poids faible correspond à $2^0 = 1$. Si ce bit est à 1, alors le nombre total est impair (car on ajoute 1 au reste du nombre qui est forcément pair). Si ce bit est à 0, alors le nombre total est pair (car on n'ajoute rien au reste du nombre qui est forcément pair).
    \end{answer}
...
def addition_binaire(A, B):
    ...
    # Initialisation de la retenue et du résultat
    retenue = 0
    resultat = ''

    # Parcours des bits de droite à gauche
    for i in range(len(A)-1, -1, -1):

        # Récupération des bits de A et B
        bit_a = int(A[i])
        bit_b = int(B[i])

        # Calcul de la somme des bits et de la retenue
        somme = bit_a + bit_b + retenue

        # Détermination du bit résultat et de la nouvelle retenue
        if somme >= 2:
            retenue = 1
            somme -= 2
        else:
        ...
        # Ajout du bit résultat au début de la chaine résultat
        resultat = str(somme) + resultat

    # Si il reste une retenue, on l'ajoute au début du résultat
    if retenue == 1:
        resultat = '1' + resultat
    return resultat
