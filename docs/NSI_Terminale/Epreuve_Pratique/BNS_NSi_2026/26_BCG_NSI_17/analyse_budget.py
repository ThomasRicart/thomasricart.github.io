...
def total_par_type(mouvement, type_mouvement):
    ''' Renvoie la somme totale des montants correspondant au type_mouvement
    Renvoie 0 si aucun mouvement ne correspond
    - mouvement: une liste de dictionnaire
    - type_mouvement (str) '''
    montant_total = 0
    for m in mouvement:
        if m['type'] == type_mouvement:
            montant_total += m['montant']
...
def solde_annuel(mouvements):
    ...
    # Modification pour aller de 1 à 12
    for m in range(1, 13):
        total = total + solde_mensuel(mouvements, m)

    return total

# Certains mouvements on été ignorés car le range dans le code n'allait que jusqu'au mois
# de novembre, ignorant celui de descembre car le range allait de 1 à 11 inclus.3
#############################################################################
# Écrire ci-dessous la fonction test_solde_annuel (Question 2)              #
#############################################################################
def calcul_solde_annuel_manuel(mouvements):
    total_annuel = 0
    for m in mouvements:
        if m['type'] == 'recette':
            total_annuel += m['montant']
        else:
            total_annuel -= m['montant']
    return total_annuel

print(calcul_solde_annuel_manuel(mouvements_test))
# affiche 150.0
print(solde_annuel(mouvements_test))
# affiche 1050.0 avant correction
# assert calcul_solde_annuel_manuel(mouvements_test) == solde_annuel(mouvements_test)
