...
def test_ecart_temperature():
    assert ecart_temperature(datas_temperature, 1900) == -0.15
    assert ecart_temperature(datas_temperature, 2042) == None

# print(test_ecart_temperature())
...
# print(derniere_annee_ecart_negatif(datas_temperature)) # réponse de la dernière année avec l'écart négatif : 1977
...
def moyenne_ecarts(annee_debut, annee_fin, datas):
    ...
    for dico in datas:
        if annee_debut <= dico["année"] and dico["année"] <= annee_fin:
            somme = somme + dico["écart"] # erreur : un - au lieu d'un +
            compteur += 1
...
def graphique(datas):
    ...
    for el in datas: # réponse à la question 4
        annees.append(el['année'])
        ordonnees.append(1)
