...
class Salle:
    def __init__(self, nom):
        ...
        '''
        self.nom = nom
        self.tresor = random.randint(1, 20)     # Trésor aléatoires entre 1 et 20
        self.pieces_suivantes = []              # Liste de pièces accessibles

    def ajouter_porte(self, autre_salle):
        ''' Ajoute une porte à une salle '''
        # A ECRIRE
        if autre_salle not in self.pieces_suivantes:
            self.pieces_suivantes.append(autre_salle)
            autre_salle.pieces_suivantes.append(self)
...
def test_classe_salle():
    ...
    B.ajouter_porte(C)
    C.ajouter_porte(D)
    print(f'Liste des pièces suivantes de A: {A.pieces_suivantes} --- Résultat attendu: [B, C]')
    print(f'Liste des pièces suivantes de B: {B.pieces_suivantes} --- Résultat attendu: [A, C]')
    print(f'Liste des pièces suivantes de C: {C.pieces_suivantes} --- Résultat attendu: [A, B, D]')
    print(f'Liste des pièces suivantes de D: {D.pieces_suivantes} --- Résultat attendu: [C]')

#test_classe_salle()

def test_ajouter_salle(nb_salles = 6):
    d = Donjon()
