...
def lire_trajets(nom_fichier):
    ...
    with open(nom_fichier, 'r', encoding='utf-8') as f:
        ...
        for ligne in lecteur:
            ...
            ### PARTIE A COMPLETER ###
            trajet['arrivee'] = ligne[1]
            trajet['duree'] = int(ligne[2])
            trajet['date'] = ligne[3]
            trajet['dist'] = float(ligne[4])
...
def trajet_le_plus_long(liste_trajets):
    ...
    """
    date_trajet_plus_long = liste_trajets[0]['date']
    distance_trajet_plus_long = liste_trajets[0]['dist']
    for trajet in liste_trajets:
        if trajet['dist'] > distance_trajet_plus_long:
            distance_trajet_plus_long = trajet['dist']
            date_trajet_plus_long = trajet['date']
...
T1 = Trajet(liste_trajets[0]['depart'],
            liste_trajets[0]['arrivee'],
            liste_trajets[0]['duree'],
            liste_trajets[0]['date'],
            liste_trajets[0]['dist'])
T2 = Trajet(liste_trajets[1]['depart'],
            liste_trajets[1]['arrivee'],
            liste_trajets[1]['duree'],
            liste_trajets[1]['date'],
            liste_trajets[1]['dist'])
T3 = Trajet(liste_trajets[2]['depart'],
            liste_trajets[2]['arrivee'],
            liste_trajets[2]['duree'],
            liste_trajets[2]['date'],
            liste_trajets[2]['dist'])
...
def distance_totale(T):
    if T.suivant == None:
        return T.distance
    return T.distance + distance_totale(T.suivant)
...
# ----------------------------------------------------------
# Protocoles de tests --- Ne pas modifier
# ----------------------------------------------------------
...
def test_question43(T):
    ...
    print(f'{distance_totale(T)} -- valeur attendue: Mairie')
