...
\begin{itemize}
    \item construire des requêtes d'interrogation à l'aide de \texttt{SELECT}, \texttt{FROM}, \texttt{WHERE} (avec les opérateurs logiques \texttt{AND}, \texttt{OR}), \texttt{JOIN... ON};
...
\begin{blocvisiblex}[height=3]
    {\quad}
        Le résultat renvoyé par la requête SQL serait une liste des \texttt{id\_vol} des vols dont l'aéroport d'arrivée est 'CDG'. En se basant sur la table \texttt{vol}, les vols correspondants seraient AI0015, AI01258, AI0292.
\end{blocvisiblex}
...
\begin{minipage}{0.6\linewidth}
    ...
    \end{tikzpicture}
    \captionof{figure}{Graphe du réseau aérien}
...
\begin{blocvisible}[height=3]
    Après l'exécution de la fonction \texttt{parcours} sur le \texttt{graphe\_airinfo} à partir de la ville \verb|'W'|, la variable \texttt{visitees1} contiendra les villes accessibles depuis \verb|'W'| dans ce graphe, soit \verb|['W', 'P', 'T', 'S', 'B']|.
...
\begin{minipage}{0.48\linewidth}
    \begin{blocvisible}[height=1.5]
    Réponse: Proposition C\@: un parcours en profondeur.
    \end{blocvisible}
\end{minipage}
