...
class Grille:
    def __init__(self):
        self.grille = [[0 for i in range(7)] for j in range(6)]
    ...
    def joue(self, colonne, joueur):
        ligne = 5
        # Tant qu'on est pas arrivé en haut ET
        # la case observée n'est pas vide
        while ligne != -1 and self.grille[ligne][colonne] != 0:
            # On cherche la case suivante (au dessus)
            ligne = ligne - 1
        # SI on n'est pas arrivé en haut, 
        # on joue le coup de joueur
        if ligne != -1:
            self.grille[ligne][colonne] = joueur
            return True
        # sinon il est impossible de jouer cette colonne
        else:
            return False
    \end{CodePythontexAlt}
...
jeu1.joue(3, 1)
    \end{CodePythontexAlt}
        ...
        \medskip
    \item Le score des joueurs sont $(15, 7)$. Le score de la grille est donc $-8$
    ...
    \item \quad
     ...
     \begin{CodePythontexAlt}[TaillePolice=\large, Largeur=1\linewidth,Centre,Lignes=true,PremLigne=1]{}
    def score(self):
        scores_joueurs = {0: 0, 1: 0, 2: 0}
        for i in range(len(self.grille)):
            for j in range(len(self.grille[i])):
                scores_joueurs[self.grille[i][j]] += valeur_case(i, j)
        return scores_joueurs[2] - scores_joueurs[1]
    \end{CodePythontexAlt}
