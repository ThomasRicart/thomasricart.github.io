...
\begin{enumerate}[resume=monDS]
    ...
    La probabilité que le module prélevé soit défectueux et provienne de la ligne A est de $0,018$.
     \end{blocvisiblex}
     \medskip
    \item Calculer la probabilité que le module prélevé soit défectueux. Montrer que cette probabilité est égale à $0,046$.
    \begin{blocvisiblex}[height=2.5]
    {\quad}
    ...
    \begin{blocvisiblex}[height=2.5]
    {\quad}
    $P_A(D)=\dfrac{P(A\cap D)}{P(D)}=\dfrac{P(A) \times P_D(A)}{P(D)}=\dfrac{0,6 \times 0,03}{0,046}\approx 0,3913$
...
Le technicien souhaite que la probabilité de détecter au moins un module défectueux sur $n$ prélèvements soit supérieure à $0,9$.

\begin{enumerate}[series=monDS, resume]
    \item Montrer que cette condition s'écrit ${0,954}^n \leq 0,1$.
...
\begin{enumerate}[series=monDS, resume]
    \item Déterminer par le calcul une expression de la fonction dérivée $f'$ et montrer que $f'(t)=-\dfrac{5}{2}\cdot \e^{-0,5t}$.
    \begin{blocvisiblex}[height=3]
    {\quad}
    $f(t)= 5 \cdot \e^{-0,5t}$
...
La fonction $f$ est donc définie pour tout $t\geq 0$ par \fbox{$f(t)=(at+b)\cdot \e^{-0,5t}$}
...
Dans la suite de l'exercice on considèrera la fonction $f$ définie sur $\fo{0}{+\infty}$ par $f(t)=5\cdot (2t+1)\cdot \e^{-0,5t}$.
...
\begin{enumerate}[series=monDS, resume]
   ...
   \item Déterminer grâce à Géogébra à partir de quelle valeur de $t$ la tension $f(t)$ devient inférieure à 1 volt. (On donnera une valeur approchée à $10^{-2}$ près.)
    \begin{blocvisiblex}[height=1]
    {\quad}
    $f(t)<1$ à partir de $t\approx 9,1$ ms.
    \end{blocvisiblex}
\end{enumerate}
...
\begin{enumerate}[series=monDS, resume]
   ...
   \begin{blocvisiblex}[height=1]
    ...
    \faSquare[regular] \, $F(a) - F(b)$ \hfill \faSquare[regular] \, $F(b) - F(a)$ \hfill \faCheck[regular] \, $F(b)$ \hfill \faSquare[regular] \, $F(a)$ \hspace{1cm} 
    \end{blocvisiblex}
    \medskip
